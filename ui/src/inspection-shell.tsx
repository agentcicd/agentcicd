...
export function InspectionShell({ projectName, activeSection, onNavigate, headerActions, children }: InspectionShellProps) {
  ...
  return <div className="flex min-h-screen bg-slate-50 text-slate-900">
    ...
    <main className="min-w-0 flex-1"><header className="flex h-16 items-center gap-3 border-b border-gray-200 bg-white px-4 lg:px-6"><button className="rounded-md p-2 hover:bg-slate-100 lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open navigation"><Menu className="h-5 w-5" /></button><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{projectName}</p></div>{headerActions ? <div className="ml-auto flex shrink-0 items-center gap-2">{headerActions}</div> : null}</header><div className="min-h-[calc(100vh-4rem)] p-4 lg:p-6">{children}</div></main>
  </div>;
}
