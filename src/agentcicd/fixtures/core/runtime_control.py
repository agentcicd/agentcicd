...
def runtime_pool_lease(
    ...
    *,
    ...
    config = _pool_lease_client_config(
        pool,
    ...
    try:
        lease = _acquire_pool_lease(config)
        ...
        if lease is not None and lease.lease_id:
            _return_pool_lease(
                ...
                node_disposition=_node_disposition_for_status(config.pool_kind, _terminal_status_for_exception(exc)),
                http_timeout_seconds=config.http_timeout_seconds,
            )
            lease = None
        raise
        ...
        if lease is not None and lease.lease_id:
            _return_pool_lease(
                ...
                node_disposition=_node_disposition_for_status(config.pool_kind, "completed"),
                http_timeout_seconds=config.http_timeout_seconds,
            )


def _pool_lease_client_config(
    pool: dict[str, Any] | None,
    *,
    request_id: str | None,
    executor_id: str | None,
    fixture_id: str | None,
) -> _PoolLeaseClientConfig | None:
    if not pool:
        return None
    base_url = os.getenv("AGENTCICD_RATE_LIMITER_BASE_URL", "").strip().rstrip("/")
    if not base_url:
        return None
    pool_config = pool.get("config") if isinstance(pool.get("config"), dict) else {}
    pool_name = str(pool.get("key") or pool.get("pool_name") or "").strip()
    pool_fixture_id = str(pool.get("fixture_id") or "").strip()
    return _PoolLeaseClientConfig(
        base_url=base_url,
        pool_name=pool_name,
        pool_kind=str(pool.get("kind") or pool_config.get("kind") or "session").strip().lower(),
        request_id=request_id or str(pool.get("request_id") or "") or _default_pool_request_id(pool_name),
        executor_id=executor_id or os.getenv("SPARK_EXECUTOR_ID", ""),
        fixture_id=pool_fixture_id or fixture_id or "",
        lease_ttl_seconds=_pool_float(
            pool,
            pool_config,
            "lease_ttl_seconds",
            "AGENTCICD_POOL_LEASE_TTL_SECONDS",
            _DEFAULT_LEASE_TTL_SECONDS,
        ),
        acquire_timeout_seconds=_pool_float(
            pool,
            pool_config,
            "wait_timeout_seconds",
            "AGENTCICD_POOL_WAIT_TIMEOUT_SECONDS",
...
def _pool_float(
    ...
    *,
    ...
    value = pool.get(key) or config.get(key)
    if value is None and fallback_key is not None:
        value = pool.get(fallback_key) or config.get(fallback_key)
    return float(value or os.getenv(env_key, default))


def _acquire_pool_lease(config: _PoolLeaseClientConfig) -> PoolLease:
    payload = _pool_lease_acquire_payload(config)
    response = _poll_pool_lease_acquire(config, payload)
    return PoolLease(
        pool_name=str(response.get("pool_name") or config.pool_name),
        pool_kind=str(response.get("pool_kind") or config.pool_kind),
        lease_id=str(response.get("lease_id") or ""),
        acquired_at=monotonic(),
...
def _pool_lease_acquire_payload(config: _PoolLeaseClientConfig) -> dict[str, Any]:
    return {
        "pool_name": config.pool_name,
        "pool_kind": config.pool_kind,
...
def _poll_pool_lease_acquire(config: _PoolLeaseClientConfig, payload: dict[str, Any]) -> dict[str, Any]:
    deadline = monotonic() + max(0.1, config.acquire_timeout_seconds)
    while True:
        try:
            return _post_json(
                f"{config.base_url}/pools/leases/acquire",
                payload,
                timeout_seconds=min(10.0, config.http_timeout_seconds),
            )
        except PoolLeaseUnavailable:
            remaining = deadline - monotonic()
            if remaining <= 0:
                raise TimeoutError(f"Timed out acquiring pool lease for {config.pool_name}") from None
            sleep(max(0.05, min(remaining, config.poll_seconds)))
    ...
    class RateLimitHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            try:
                ...
                path = self.path.rstrip("/")
                response = _runtime_control_response(path, payload, manager, pool_manager)
...
def _runtime_control_response(
    ...
    manager: DriverLeaseManager,
    pool_manager: DriverPoolLeaseManager,
) -> dict[str, Any]:
    route = _RUNTIME_CONTROL_POST_ROUTES.get(path)
    if route is not None:
        return route(payload, manager, pool_manager)
    if path.startswith("/pools/nodes/") and path.endswith("/heartbeat"):
        ...
        return _handle_pool_node_heartbeat(payload, pool_manager, node_id)
    if path.startswith("/pools/leases/") and path.endswith("/heartbeat"):
        ...
        return _handle_pool_lease_heartbeat(payload, pool_manager, lease_id)
    if path.startswith("/pools/leases/") and path.endswith("/return"):
        ...
        return _handle_pool_lease_return(payload, pool_manager, lease_id)
    raise FileNotFoundError("not found")


def _handle_rate_limit_acquire(
    payload: dict[str, Any],
    manager: DriverLeaseManager,
    pool_manager: DriverPoolLeaseManager,
) -> dict[str, Any]:
    ...
    if max_in_flight is not None:
        manager.configure(str(payload.get("key") or _DEFAULT_LIMITER_KEY), int(max_in_flight))
    lease = manager.acquire(
        ...
        wait_timeout_seconds=float(payload.get("wait_timeout_seconds") or _DEFAULT_WAIT_TIMEOUT_SECONDS),
    )
    return {
...
def _handle_rate_limit_release(
    payload: dict[str, Any],
    manager: DriverLeaseManager,
    pool_manager: DriverPoolLeaseManager,
) -> dict[str, Any]:
    _ = pool_manager
    return {"released": manager.release(str(payload.get("lease_id") or ""))}


def _handle_rate_limit_renew(
    payload: dict[str, Any],
    manager: DriverLeaseManager,
    pool_manager: DriverPoolLeaseManager,
) -> dict[str, Any]:
...
def _handle_rate_limit_configure(
    payload: dict[str, Any],
    manager: DriverLeaseManager,
    pool_manager: DriverPoolLeaseManager,
) -> dict[str, Any]:
...
def _handle_pool_node_register(
    payload: dict[str, Any],
    manager: DriverLeaseManager,
    pool_manager: DriverPoolLeaseManager,
) -> dict[str, Any]:
...
def _handle_pool_node_heartbeat(
    payload: dict[str, Any],
    pool_manager: DriverPoolLeaseManager,
    node_id: str,
) -> dict[str, Any]:
...
def _handle_pool_lease_acquire(
    payload: dict[str, Any],
    manager: DriverLeaseManager,
    pool_manager: DriverPoolLeaseManager,
) -> dict[str, Any]:
    ...
    return _pool_lease_response(lease)


def _pool_lease_response(lease: PoolLease) -> dict[str, Any]:
    return {
        "lease_id": lease.lease_id,
...
def _handle_pool_lease_validate(
    payload: dict[str, Any],
    manager: DriverLeaseManager,
    pool_manager: DriverPoolLeaseManager,
) -> dict[str, Any]:
...
def _handle_pool_lease_heartbeat(
    payload: dict[str, Any],
    pool_manager: DriverPoolLeaseManager,
    lease_id: str,
) -> dict[str, Any]:
...
_RUNTIME_CONTROL_POST_ROUTES = {
    "/rate-limit/acquire": _handle_rate_limit_acquire,
    "/rate-limit/release": _handle_rate_limit_release,
    "/rate-limit/renew": _handle_rate_limit_renew,
    "/rate-limit/configure": _handle_rate_limit_configure,
    "/pools/nodes/register": _handle_pool_node_register,
    "/pools/leases/acquire": _handle_pool_lease_acquire,
    "/pools/leases/validate": _handle_pool_lease_validate,
}
