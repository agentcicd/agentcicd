...
def _resolve_row_call_args(func: Any, provided_args: tuple[Any, ...]) -> tuple[Any, ...]:
    ...
    for param in params[len(resolved):]:
        ...
        if annotation is TimeoutConfig or annotation_name in {
            "TimeoutConfig",
            "agentcicd.fixtures.core.timeout.TimeoutConfig",
        }:
            resolved.append(TimeoutConfig())
        elif annotation is RetryConfig or annotation_name in {
            "RetryConfig",
            "agentcicd.fixtures.core.retry.RetryConfig",
        }:
...
def _require_pyarrow() -> Any:
    try:
        ...
        raise ModuleNotFoundError(
            "pyarrow is required to execute agentcicd.fixtures UDF batches. "
            "Install agentcicd.sandbox in runtime environments."
        ) from exc
    return pa
